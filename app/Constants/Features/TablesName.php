<?php

namespace App\Constants\Features;

class TablesName
{

    public const FEATURES = 'features';

    public const VENUES = 'venues';
    public const VENUE_CATEGORIES = 'venue_categories';
    public const VENUE_RESTRICTION_CATEGORY = 'venue_restriction_category';
    public const VENUE_LABELS = 'venue_labels';

    public const ADDRESS_VENUES = 'address_venues';



    public const RESTRICTION_CATEGORIES = 'restriction_categories';
    public const LABELS = 'labels';
    public const ADDRESSES = 'addresses';



    public const BUILDINGS = 'buildings';
    public const BUILDING_CATEGORIES = 'building_categories';
    public const BUILDING_LABELS = 'building_labels';
    public const ADDRESS_BUILDINGS = 'address_buildings';




    public const FOOTPRINTS = 'footprints';
    public const FOOTPRINT_BUILDING = 'footprint_building';
    public const FOOTPRINT_CATEGORIES = 'footprint_categories';
    public const FOOTPRINT_LABELS = 'footprint_labels';



    public const LEVELS = 'levels';
    public const LEVEL_CATEGORIES = 'level_categories';
    public const LEVEL_BUILDING = 'level_building';
    public const LEVEL_LABELS = 'level_labels';
    public const ADDRESS_LEVELS = 'address_levels';
    
    
    public const UNITS = 'units';
    public const UNIT_CATEGORIES = 'unit_categories';
    public const UNIT_ACCESSIBILITY = 'unit_accessibility';
    public const UNIT_LABELS = 'unit_labels';
    public const UNIT_LABEL = "unit_labels";
    
    public const ACCESSIBILITY_CATEGORIES = 'accessibility_categories';

    public const ANCHORS = "anchors";
    public const ADDRESS_ANCHORS = 'address_anchors';


    public const AMENITIES = "amenities";
    public const AMENITY_CATEGORIES = "amenity_categories";
    public const AMENITY_ACCESSIBILITY = "amenity_accessibility";
    public const AMENITY_UNIT = "amenity_unit";
    public const ADDRESS_AMENITIES = "address_amenities";
    public const AMENTITY_LABEL = "amenity_labels";






}


